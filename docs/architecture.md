...
| `getOpposingStaffPill` (Weak / Elite staff) | [src/lib/mlb/analysis.ts](../src/lib/mlb/analysis.ts) | The underlying signal (`staffEra`) is already folded into batter R via `staffMod` and pitcher P(W) via the bullpen multiplier. The pill is a UI cue. |
