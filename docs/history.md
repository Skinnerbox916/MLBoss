...
**What was actually wrong — five things, one screen.**

1. **The batter 0–100 scale was never centred.** `CATEGORY_BASELINE_CONFIG.normRange` windows were "floor → elite" guesses (AVG .220–.300, R .090–.155 …) with the league mean sitting at 0.11–0.39 of each window. A batter with exactly league-average rates, against a league-anchor starter, in a neutral park, unknown lineup spot, composited to **29** on six categories and **38** on nine — the docstring said 50. The pitcher side had this exact disease and fixed it in [2026-07](#2026-07--pitcher-score-scale-recentered-league-average--50); the fix never crossed to the batter side. Tier bands ("neutral" 45–55) therefore described well-above-average days.
2. **Weight collapse made the ceiling reachable.** Pivotality weights on the streaming page put ~all weight on the one contested category (AVG in the surfaced league). The composite then equals `100 × norm(AVG) × opportunity`, and with a #2 hitter's 1.13× opportunity anything at norm ≥ 0.88 — expected AVG ≥ .290 on the old window — rendered 100. Coors vs a weak SP took a *league-average* batter to .286.
3. **The matchup layer out-muscled talent.** After the [prior-N refit](#2026-09--category-regression-priors-were-4-9x-too-weak-and-were-never-measured) compressed talent spread ~40%, the still-unshrunk modifiers dominated: swapping only the opposing SP moved that average batter 17 → 56; adding a hitter park, 72. The R/RBI pitcher knob sat pinned at its ±20% clamp against a below-average staff, across a norm window only ±28% wide. The per-knob fit had already said the opposing-pitcher knob delivers 0.36–0.77 of its swing and park 0.34–0.77; `knobReliability.ts` was built to hold those numbers and shipped at 1.0 pending a cohort regenerated on the neutralised baseline.
4. **The A's park lookup missed.** `parks.ts` keyed Sutter Health as `OAK`; MLB, Yahoo and ESPN all say `ATH`; `normalizeTeamAbbr` had no alias. So `getParkByTeam(normalizeTeamAbbr('ATH'))` was `undefined`, every A's batter skipped the baseline neutralisation the [2026-09 fix](#2026-09--modifier-reliability-given-one-home-and-the-park-double-count-fixed-at-the-baseline) added (charged for Sutter twice at home, never discounted on the road) and got no home-park dose on the roster page. The 2026.09.05 bump had found and fixed the identical AZ/ARI miss and stopped one short. It was the only mismatch between MLB's 30 abbreviations and the park table.
5. **The league-mean anchors were stale**, and with fitted `leaguePriorN` of 425–900 the prior *centre* now matters far more than it did at N = 100. 2026 season-to-date (MLB Stats API hitting aggregate, 30 teams, 163,893 PA, 2026-09-08): AVG .2437 vs .239, HR .0302 vs .0275, TB .3552 vs .340, **SB .0178 vs .010** — the SB anchor was a pre-2023 rate the bigger bases roughly doubled. K and BB were right.

**What changed.**

- `norm: { center, halfWidth }` replaces `normRange: [lo, hi]`. Centre = `leagueMean`, so 50 = league average by construction. Half-width = **2.5 forecast-SDs**: the SD of `buildBatterForecast`'s own per-PA output per category across the regenerated retro cohort, after the reliabilities below — one SD of forecast movement is worth the same score points in every category, ~1% of batter-days clip. (Widths were not simply preserved as on the pitcher side because they had never been fitted; the cohort makes them measurable.) A `leagueAverage` profile in the batter harness locks the composite to 46–54; it reads 51.
- `KNOB_RELIABILITY` takes the fitted exponents from the regenerated cohort (37,386 graded rows, rate basis): opposing pitcher AVG/H 0.36, TB 0.51, HR 0.76, R/RBI 0.41/0.42, K 0.77, BB 0.64; park AVG/H 0.36, TB 0.60, HR 0.48, R/RBI 0.71/0.66, K 0.52, BB 0.39; team-SB 0.53. Table: [forecast-verification.md](./forecast-verification.md#regenerated-cohort-2026-09-08--the-fit-that-set-the-reliability-table).
- League means refreshed in all three homes together (`categoryBaselines` `leagueMean`, `batterForecast` log5 anchors, `LEAGUE_XBA`).
- `ATH` is the canonical Athletics abbreviation (`OAK` aliases to it; `parks.ts` re-keyed).
- Two level biases found while checking that "league-average in, league-average out" actually held: the talent-path AB/PA shortcut `1 − BB%` (HBP/SF/SH are 2.3% of PA; every talent-path H and TB was 2.6% high — the comment claimed "< 0.5%"), and the HR knob comparing a talent-basis SP rate (.024 at league average) against the actual-basis league .030, which read a dead-average starter as a 20% HR suppressor. `abPerPA` / `NON_BB_NON_AB_PER_PA` and `SP_HR_TALENT_TO_ACTUAL`; rationale in [league-baselines.md](./league-baselines.md#batter-hr-knob-basis).

**Effect.** Every batter score moves, and the movement is *not* uniform this time (unlike the pitcher recentring): the reliabilities compress matchup swings, the anchors shift baselines, and the SB/HR anchors move some players more than others. On the retro cohort (posted-lineup starters, so a shade better than the league as a whole) the ten-category day score now reads p2 32 / median 54 / p98 83, with 0.7% of batter-days at 99+; the old build's median was 43 with league-average at 29–38. Rankings inside a page reorder where the matchup layer had been over-weighting park and SP — which is the point. Forecast quantities (`predicted.*`) change through items 2–5, so all four batter engines segment in the ledger; the pitcher engine is untouched except points-pitcher H through `talentHitsPerPA`.

**Verified.** Batter harness 13/13 (`leagueAverage` 51), pitcher harness all-pass, `tsc`/`next lint` clean; the cohort was regenerated a second time with the table in place and the fit read back **1.00–1.05 on every set knob** (pitcher 1.00–1.05, park 0.99–1.04, team-SB 1.00; talent 1.03–1.13, HR 1.29), and the shipped half-widths matched the rebuilt cohort's 2.5 SD to within rounding.

**Don't reintroduce:** (a) "floor → elite" normalisation windows, or any window whose centre is not the league mean — the harness `leagueAverage` profile exists to catch it; (b) a reliability value for a knob before its home/away split has been checked (a double-count masquerades as a scaling miss); (c) `1 − BB%` as AB/PA; (d) comparing a talent-basis pitcher primitive against an actual-basis league rate — check the basis of both sides of every log5/ratio; (e) a hand-keyed park table entry without an alias in `teamAbbr.ts` — the check is one line: every MLB abbreviation must resolve to a park key.

**Left open, with numbers:** `LEAGUE_HR_PER_CONTACT` .035 is stale (2026 HR/(PA−K−BB) ≈ .044) — a pitcher-engine anchor, to be moved on pitcher-side evidence ([league-baselines.md](./league-baselines.md#batter-hr-knob-basis)). `LEAGUE_XSLG` .404 was not re-sourced (2026 SLG is .400; xSLG runs a touch above). Batter talent HR now reads slightly under-spread (1.24). The single-contested-category pile-up is structural — with one category the day score *is* that category's normalised value times opportunity — and is now confined to genuinely +2 SD days rather than anything above .290.
...
- **An unposted slot on a day MLB has already posted** is not a missing turn — see the 2026-07-25 amendment below.

**Amendment 2026-07-25 — gate the inference on the day's posting state.** As shipped, inference fired on *any* unposted slot anywhere in the remaining window. On 2026-07-25 (wk 17, 2 days left) that handed four idle rostered starters a phantom ~5.5 IP start each: the Boss Card read "~45 IP left" against a true 27.8, and the Boss Brief — which summed posted probables itself — printed "projected for 125.5 IP" in the same render. Probables post ~7-10 days out (live that day: D+1 = 1/30 slots unposted, D+7 = 2/30, D+14 = 30/30), so inside the horizon an empty slot is a real TBD — a bullpen game or undecided spot start — and awarding it to a rostered ace invents IP, K, W, and QS that corrupt every corrected margin downstream. `projectPitcherPlayer` now claims only on days whose slate is ≥50% unposted (`isSlateUnposted`), never today. The Cole-Sunday case the inference exists for is unaffected: that day was broadly unposted. **Don't reintroduce:** a fixed D+N cutoff in place of reading the slate — the posting horizon moves during the day and by team.
...
**Follow-on (SV projection):** `projectRelieverPlayer` now projects saves from observed pace (`observedSavesPerAppearance` × window appearances; overall-line `saves`/`gamesPitched` plumbed through `ActivePitcher`). Before this, SV was never projected on the categories side — the matchup SV row fell back to MTD and read a phantom `0=0` tossup even against an opponent rostering a closer. SV is now a projectable pitcher counting cat (`COUNTING_PITCHER_STAT_IDS`), but no starter contributes (SV isn't in `PITCHER_NORM`, so it never enters a starter's rating). A save-less roster projects 0 → SV reads a real deficit and auto-concedes when the opponent's projected saves are large enough; a weak-saves opponent leaves it chase-able (deliberate — a reachability floor to force-concede it was considered and rejected as over-engineering; the user concedes manually). SV carries no `streamCapacity` entry — SP streams don't make saves.
...
**Replaced with:** [projectRosterWeek](../src/lib/points/rosterWeek.ts) — the day-solve aggregation extracted into one engine, consumed by both `analyzeTeam` (week projection) and `analyzePointsStreaming` (coverage + marginal-re-solve bases, behavior-identical). Injected day scorer per the `optimizePointsLineup` contract, so each caller still prices a batter-day in its own context (streaming: matchup-adjusted observed-spot; analyzeTeam: same, injured→0). Pitchers stay summed by probable starts. The engine OWNS the future-window roster neutralization (`is_editable: true`, `starting_status` cleared, status-injured players in active slots benched) — previously each caller had to remember this (streaming did via `futureRoster`; the first analyzeTeam wiring didn't, which would pin a mid-slate locked lineup across all future days). Don't pass pre-cooked "projectable" rosters from callers; the engine normalizes.

**Don't reintroduce:** a roster-sum or top-K week projection in `analyzeTeam` (or anywhere). The optimal-lineup day-solve is the canonical points weekly batting projection; if it's too slow somewhere, memoize the scorer (streaming does), don't re-approximate. Per-player `thisWeekPoints` staying 0 for injured players is still required — the engine benches them in the aggregate, but the row facts are consumed elsewhere.
