...
| **Retro reconstructions** (2026-09-04) | Postgres, `forecast_snapshots` rows whose `engine` starts with `retro-` | The unchanged engines re-run on as-of inputs for past dates. They share the ledger table (same identity, same grading join) but are **not** ledger by the rubric: every input is still here (corpus, MLB game logs, engine code), so a row can be recomputed at will. Contract: a retro capture of a date **replaces** that date's rows for the engines it ran (upsert on the identity + sweep of untouched rows, `src/lib/retro/capture.ts`) and stamps the current `MODEL_VERSION`; live rows in the same table stay first-write-wins forever. Rationale: [forecast-verification.md](./forecast-verification.md#retro-rows-are-reconstructions) |

Rule of thumb: if losing it means waiting for the world to repeat itself, it's ledger (or an `obs:` key when it's a small decaying signal); if you can refetch it, it's cache — and if you can *recompute* it from things you still hold, it is a reconstruction, replaceable even when it sits in a ledger table. Feature code never talks to `pg` directly — go through the modules in `src/lib/db/` (users, prefs) and `src/lib/ledger/` (forecast verification, see [forecast-verification.md](./forecast-verification.md)).
