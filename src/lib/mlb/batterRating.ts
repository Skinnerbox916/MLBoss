...
// L2 per-PA forecast logic now lives in `./batterForecast.ts`. This
// module is L3 — normalization, weighting, composite multipliers, tier.
...
export function getBatterRating(args: BatterRatingArgs): BatterRating {
  ...
  for (const cat of supported) {
    const f = forecast.perCategory[cat.stat_id];
    if (!f) continue;
