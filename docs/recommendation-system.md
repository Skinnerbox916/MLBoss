...
**Stream capacity (2026-07):** for pitcher counting cats (K/W/QS/IP) a losing corrected margin is computed on the gap **net of what the remaining moves budget could add via streams** (capped at even — reachability, never a projected lead; rows softened this way carry `streamAssisted` and the Game Plan tile reads "in reach via streams"). Without this the loop is circular: the current-roster projection says "out of reach" → auto-concede zeroes the pivotality weight → the streaming board stops valuing the very cats streaming would win, while the Boss Brief simultaneously says "stream N to catch up". Mechanics + per-stream yields: `AnalyzeOpts.streamCapacity` in [matchup/analysis.ts](../src/lib/matchup/analysis.ts) and `LEAGUE_AVG_START_OUTPUT` in [projection/streamPitcherCatImpact.ts](../src/lib/projection/streamPitcherCatImpact.ts). Deliberately counting-cats-only (volume can't reliably fix ERA/WHIP; batter adds displace), and deliberately **my-side-only** — capacity models a lever the user controls, not a prediction. The opponent's streaming is modeled where a prediction belongs instead: as expected volume inside their projection, always ([projection.md#expected-streamed-starts](./projection.md#expected-streamed-starts)). Mid-week that stays clean of capacity (my rows are roster-only, so capacity is the full remaining budget); on the next-week pivot my own expected streams are priced into the rows, so capacity there passes the **residual** (cap − expected) to avoid counting the same moves twice.
...
| `LineupRosterCard` rules | [src/components/dashboard/cards/LineupRosterCard.tsx](../src/components/dashboard/cards/LineupRosterCard.tsx) | Roster + lineup state | Health / eligibility / IL-slot issues. Orthogonal to matchup state — these rules answer "is your lineup mechanically broken", not "what should you chase" |
