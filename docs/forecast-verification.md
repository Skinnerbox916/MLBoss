...
| Start-probability fit | `scripts/retro-start-probabilities-fit.ts [--cut=]` | Fits the QS / W reach model (P(reach 6 / 5 IP) with the leash, P(ER ≤ 3 \| 6 IP), W credit) from the graded retro pitcher cohort with team results from the MLB schedule; a HOLDOUT block (fit before the cut, score after against the captured forecasts) and the FULL coefficients to paste | See [unified-rating-model.md](./unified-rating-model.md#start-probabilities) |
| Pitcher prior fit | `scripts/retro-pitcher-shrinkage-fit.ts [--through=]` | Fits the pitcher talent regression priors (K, BB, xwOBACON, hard-hit, HR/contact, IP/start) out of sample from the corpus: window A rate → later window B outcome, current + the real prior season (2025 Savant + MLB starter split) + league, at three split dates | See [unified-rating-model.md](./unified-rating-model.md#pitcher-regression-priors); a correctly-set engine reads gain-vs-now ≈ 0 |
...
- **Talent: IP and PA are under-spread** (1.53 / 1.45 — `ipPerStart` is too compressed; pitchers who go deep go deeper than forecast), **HR is badly over-spread** (0.44 — the `hrPerContact` talent, the pitcher twin of the batter R/RBI finding), ER 0.68, K 0.94, BB 0.82. These are talent-layer items and belong to the prior-strength work, not to the matchup layer. The contact / HR / BB priors were fitted 2026-09-23 ([unified-rating-model.md](./unified-rating-model.md#pitcher-regression-priors)); IP/start is left for the volume-model rework.
