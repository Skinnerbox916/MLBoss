...
**Replaced with:** one `MatchupContext` shape (in [src/lib/mlb/matchupContext.ts](../src/lib/mlb/matchupContext.ts)), one `Rating` discriminated union (in [src/lib/rating/types.ts](../src/lib/rating/types.ts)) with an `engine: 'batter' | 'pitcher'` field. One `ScoreBreakdownPanel`. (A unified `CompareTray` component was created in this pass but never wired into a live UI surface; the file was later deleted.) See [unified-rating-model.md](./unified-rating-model.md).
