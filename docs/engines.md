...
- Expected streamed starts (`computeTeamStreamRates` / `proRateStreamStarts` / `applyExpectedStreams` / `applyExpectedStreamsToProjection`) — [src/lib/projection/streamVolume.ts](../src/lib/projection/streamVolume.ts) — per-team demonstrated SP-add rate (recency-weighted, from the league transaction feed, capped by `maxWeeklyAdds`) folded into a projected pitcher week at league-average per-start output, categories AND the IP totals. Applied by SIDE: opponent always (pro-rated mid-week), me on the next-week pivot only — mid-week my adds are `streamCapacity`'s lever and would double-count. Points twin: `expectedStreamPoints`. See [projection.md#expected-streamed-starts](./projection.md#expected-streamed-starts)
...
- Lineup issues rules (`LineupRosterCard`) — [src/components/dashboard/cards/LineupRosterCard.tsx](../src/components/dashboard/cards/LineupRosterCard.tsx) (orthogonal: health / eligibility checks, not matchup-state recommendations)
...
- Expected stream points (`expectedStreamPoints`) — [src/lib/points/streamVolume.ts](../src/lib/points/streamVolume.ts) — points twin of `applyExpectedStreams`: the SHARED demonstrated SP-add rate and pro-rating (`computeTeamStreamRates` / `proRateStreamStarts`) priced with this league's own FA-pool mean per start (`PointsStreamingAnalysis.faStartPointsAvg`), never a constant. Same by-side rule as categories. Consumed by `usePointsOpponentWeek` (marquee projected final + points brief) and the points Next Week card. See [points-leagues.md](./points-leagues.md#opponent-stream-volume)
- Points Boss Brief (`getPointsBrief`) — [src/lib/points/brief.ts](../src/lib/points/brief.ts) — L7-analog of `getBossBrief`: one tactical line for the points dashboard marquee from live score + both sides' projected remaining (opponent side priced by `analyzePointsTeam` with `includeFA=false`, plus their expected stream points — see above); thresholds normalized to remaining volume, H2H only (the marquee renders a season variant itself)
- Matchup-adjusted points rates (`adjustedBatterPointsPerPA`, `adjustedPitcherStartPoints`, `meanTeamOffense`) — [src/lib/points/matchupAdjust.ts](../src/lib/points/matchupAdjust.ts) — L2-analog, the designed swap-in day scorer. NO new matchup math: batters re-dot the canonical `buildBatterForecast` adjusted rates (park / platoon / opp staff / weather) with the league weights; pitchers apply the `buildGameForecast` context-vs-neutral RATIO to the talent-anchored per-start baseline, with the neutral twin anchored to the slate-mean offense (the static `teams.ts` league anchors are calibrated for the relative 0–100 scale, not absolute points — a stale anchor showed up as a +12% systematic pitcher boost before the slate-mean fix). The pitcher ratio's W term carries the 2026-07 decomposed P(W): own-team run support and opposing-SP talent ride the real twin only (pens and home edge cancel — both twins share the real teams and `isHome`); see [unified-rating-model.md#start-probabilities](./unified-rating-model.md#start-probabilities).

Matchup-adjustment boundary (deliberate): applied ONLY to day/week lineup-decision scorers — lineup day scores (`analyzeTeam` / client `lineupScoring`), streaming coverage / plugs / per-start stream points. Roster-construction values (`weeklyPoints`, VOR, `thisWeekPoints`, suggested season swaps) stay talent-neutral, matching the roster page's matchup-vacuum philosophy: park context tells you who to START this week, not who to OWN.
