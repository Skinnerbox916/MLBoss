...
What's actually in the code right now (post-pull): `isGamePlanSitWorthy` keeps the **direction guard** added in [e7178ab](https://github.com/Skinnerbox916/MLBoss/commit/e7178ab) — sit-for-ratio engages only when a manageable cat is contested *and being lost* (`margin ≤ 0`), preventing the empty-lineup regression on winning weeks. Beyond that, the per-bat sit math is conservative pivotality-weighted with no stopping condition; weeks where the user is genuinely losing K/AVG can still over-sit, and the user manages that by manually conceding the cat for the week (the panel's concede/contest toggle).

Don't reintroduce a sit refinement that doesn't address all three: (a) the locked-counting fragility (gate on week confidence), (b) a per-cat opponent-total bar with a stopping condition, (c) refusal to sit when the target can't be reached. Any one of these in isolation is the failure mode that pulled this attempt.
