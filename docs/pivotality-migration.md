...
- [x] **Phase 4 — Sit/optimizer in pivotality terms** *(conservative; refinement parked)*. `sitValue.ts` refactored: `categoryWeight` ternary + `LOCKED_WIN_RESIDUAL` retired; every cat weights by its `categoryWeights[id]` (un-renormalized pivotality, 0 if conceded). Opponent-AVG anchor kept. `isGamePlanSitWorthy` reframed to "low-value counting + contested ratio/K" on weight thresholds. A stronger sit-target boost for contested manageable cats was tried and rejected — it over-sat (benched the whole lineup; no stopping condition). The proper opponent-total / sit-just-enough refinement is parked as a deliberate next step in **[docs/sit-to-flip-prd.md](sit-to-flip-prd.md)**.
