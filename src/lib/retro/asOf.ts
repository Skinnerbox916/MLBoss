 ...
 * BB includes IBB; whiffs include foul tips; hard-hit% is over TRACKED
 * batted balls; xwOBA uses est_woba on tracked BIP and the actual woba_value
...
interface AggRow {
  ...
  xwoba: number | null; woba: number | null; xba: number | null; xslg: number | null;
...
async function aggregate(kind: 'batter' | 'pitcher', season: number, asOf: string): Promise<{ rows: AggRow[]; coverage: AsOfCoverage }> {
  ...
  const res = await db.execute(sql`
    ...
    select ${who} as id,
      ...
      (count(*) filter (where is_bip and launch_speed >= 95))::float / nullif(count(*) filter (where is_bip and launch_speed is not null), 0) as hardhit,
  ...
  const rows = (res.rows as Record<string, unknown>[]).map(r => {
    ...
    return {
      ...
      xwoba: n('xwoba'), woba: n('woba'), xba: n('xba'), xslg: n('xslg'), xwobacon: n('xwobacon'), hardhit: n('hardhit'),
...
export async function pitchersAsOf(asOf: string, season: number, xera: XeraMapping | null): Promise<AsOfResult<StatcastPitcher>> {
  ...
  for (const r of rows) {
    ...
    out.set(r.id, {
      ...
      xwoba: r.xwoba, era: null, woba: r.woba, pa: r.pa, bip: r.bip,
...
export async function battersAsOf(asOf: string, season: number): Promise<AsOfResult<StatcastBatter>> {
  ...
  for (const r of rows) {
    if (r.pa === 0) continue;
    out.set(r.id, {
      mlbId: r.id, xba: r.xba, xslg: r.xslg, xwoba: r.xwoba, woba: r.woba, pa: r.pa, bip: r.bip,
