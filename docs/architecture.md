...
4. **Smoke test if it's wide-blast.** Constants that touch many engines (league means, prior strengths) need a smoke harness or test that asserts the resulting ratings stay within expected bands. Currently lives at `src/app/api/admin/test-pitcher-eval/route.ts` for pitcher constants. This is the *point-in-time* check ("do ratings still look sane right now"); its longitudinal sibling is the forecast ledger (next rule).
5. **Bump `MODEL_VERSION`, add a changelog entry, check the scorecard.** Any change that alters what an engine predicts — a constant, a league mean, a prior strength, a new modifier — must bump `MODEL_VERSION` (`src/lib/ledger/modelVersion.ts`) **and add a `MODEL_CHANGELOG` entry naming which engines/stats it touched**. Snapshots are stamped with the version, so the scorecard can segment before/after instead of blurring both into one average; the changelog is what keeps a bump from over-segmenting — untouched metrics keep pooling across it, only touched ones reset (see [forecast-verification.md](./forecast-verification.md#model-versions--the-change-manifest)). Skip either and you silently poison the comparison. Never delete old-version snapshots to "get fresh data" — they're the before-half of the proof, and the scorecard already reads only the live cohort. After the change ships and a week or two of games grade, read the by-model-version segment on `/admin/forecast` to confirm the tune actually helped. The ledger is *how we know a calibration change was an improvement* — see [forecast-verification.md](./forecast-verification.md#the-improvement-loop) for the detect → localize → fix → verify loop and the finding-shape → constants-file map that tells you *which* constant a miss implicates.
