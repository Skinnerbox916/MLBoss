...
- [ ] **Phase 6 — Delete + document.** Remove the `Focus` union, `useSuggestedFocus`, `focusToCategoryWeights` bridge, the dead `focus` field on `CategoryContribution` / `PitcherCategoryContribution`, and the now-unused `focusPanel.tsx` building blocks once `RosterFocusPanel` is converted (L6). Rewrite `recommendation-system.md` and `unified-rating-model.md` around pivotality; update `engines.md`. Run the smoke harness (`/api/admin/test-pitcher-eval`), `tsc`, `eslint`, browser-smoke all four pages.
