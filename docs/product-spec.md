...
- **Roster** — tabs for Batters and Pitchers. The page is forward-looking by design: `CategoryFocusBar` (manual chase/punt) seeds the talent-blended scoring, `DepthChart` exposes structural gaps, and `SwapSuggestions` ranks net-positive moves through the same engine. There is intentionally no top-of-page rank strip — YTD ranks are warped by injuries and cold streaks and would contradict the suggestion logic. League rankings live one click away on the **League** page. Batters tab is a full depth-chart + swap optimizer; Pitchers tab lists rostered + available pitchers (full pitcher optimizer is on the roadmap).
