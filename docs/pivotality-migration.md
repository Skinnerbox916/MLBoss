...
- [x] **Phase 3 — Weight production (the brain)** *(matchup pages only; L6 deferred)*. New `useCategoryWeights` hook produces pivotality weights + concede/contest state per side from the matchup analysis (`src/lib/hooks/useCategoryWeights.ts`). `LineupManager`, `StreamingManager`, `TodayPitchers` switched off the Phase-2 bridge to pass explicit `categoryWeights`. **Deferred to L6:** `forwardFocus` migration to `pivotality(movesToTarget-distance)` — the roster page still uses `useSuggestedFocus`. **Not yet done** (sweep up with /roster or Phase 6): explicit `analyzeMatchup` cleanup of `suggestFocus` / `suggestedFocus` / `LOCKED_THRESHOLD` (still exist but unused by converted pages); `blendedCategoryScore` renormalization.
- [x] **Phase 4 — Sit/optimizer in pivotality terms** *(conservative)*. `sitValue.ts` refactored: `categoryWeight` ternary + `LOCKED_WIN_RESIDUAL` retired; every cat weights by its `categoryWeights[id]` (un-renormalized pivotality, 0 if conceded). Opponent-AVG anchor kept. `isGamePlanSitWorthy` gates on (low-value counting weight) AND (contested ratio/K weight) AND (ratio/K margin ≤ 0 — direction guard restoring the old `chase` semantics that the weights-only gate had lost). A stronger sit-target boost for contested manageable cats was tried and rejected — it over-sat (benched the whole lineup, no stopping condition). The "stop when you've sat enough" problem was later resolved by the endgame rewrite (`computeSitPlan` replaced `isGamePlanSitWorthy`); see [history.md](history.md) "2026-06 — Endgame rewrite of auto-sit".
- [x] **Phase 5 — UI collapse** *(matchup pages only; roster panel deferred)*. `GamePlanPanel` rebuilt to **In play** (ranked by pivotality) + **Conceded** shelf with a 2-state concede/contest toggle per tile (`src/components/shared/GamePlanPanel.tsx`). Decided wins stay in-play with a small weight, never on the shelf. `ScoreBreakdownPanel` carries `categoryWeights` through so the breakdown agrees with the displayed score. **Deferred to L6 / streaming polish:** `RosterFocusPanel` still uses the old `FocusSectionTrio` building blocks in `focusPanel.tsx`; `BatterStreamingBoard` lost its chase/punt pill tinting (neutral for now). **Not yet done:** `bossBrief` and `BossCard` highlight retarget — they still read the legacy fields and work, but the prose hasn't been rewritten around pivotality.
- [~] **Phase 6 — Delete + document.** *L6/roster half DONE (2026-07, roster-value rebuild — see history.md "Roster value rebuilt"):* `RosterFocusPanel` converted to In-play/Conceded on leverage weights (`useRosterCategoryWeights` + `rosterValue.ts` — pivotality on RUPM moves-from-a-winning-rank, exactly as specced in "Roster side (L6)" above); `useSuggestedFocus`, `focusPanel.tsx` (FocusResetButton moved into `GamePlanPanel`), `forwardFocus.ts`, and `blendedCategoryScore` deleted. *Remaining (L5-surface only):* the `Focus` union + `focusToCategoryWeights` bridge in the rating engines, the `focus` field on `CategoryContribution` / `PitcherCategoryContribution`, `analyzeMatchup`'s `suggestFocus`/`suggestedFocus` emission, the streaming boards' Focus-typed props, and the bossBrief/BossCard legacy reads. These are one cleanup sweep with the bossBrief prose retarget.
