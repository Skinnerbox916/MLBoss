/**
 * Compose YTD scoreboard MatchupRows with forward batter projections to
 * produce a corrected per-cat row set that `analyzeMatchup` can ingest
 * unchanged.
 *
 ...
 *
 * Scope: ONLY batter-side rows (`isBatterStat === true`). Pitcher rows
 * pass through unchanged because v1 has no pitcher-team projection
 * engine. This is enforced inside `composeCorrectedRows` so future use
 * sites can't accidentally project pitcher cats.
 */
...
export interface CorrectedRowsInput {
   ...
   *  projection failed / hasn't loaded yet. */
...
export function composeCorrectedRows({
  ...
  return baseRows.map(row => {
    if (!row.isBatterStat) return row;
 ...
 * (`expectedPA`, already scaled by AB_PER_PA at projection time).
 *
...
function blendAvg(
  ...
  if (!proj || proj.expectedPA <= 0) return ytdAvg;
  const projH = proj.expectedCount;
  const projAB = proj.expectedPA;
