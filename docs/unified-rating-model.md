...
- **Expected-stat blend** (AVG / H / TB): the talent rate comes from a Statcast expected model, blended at `XSTAT_BLEND_WEIGHT` (0.6) with the raw actual-rate path rather than replacing it:
  ...
  - **H** ← `talent.xba × abPerPA(bbRate)` (per-PA hit rate; AB/PA = 1 − BB% − 2.3% for HBP/SF/SH — the `1 − BB%` shortcut ran H and TB 2.6% high for every batter until 2026-09-08)
  - **TB** ← `talent.xslg × abPerPA(bbRate)` (SLG is TB/AB, so TB/PA = xSLG × AB/PA)
...
**Regression strength is fitted, not chosen.** Each category's `leaguePriorN` — the plate appearances of league-average the raw blend mixes in — comes from `scripts/retro-talent-shrinkage-fit.ts`, which searches out of sample for the N that best predicts a held-out later window. Values run from 50 (K) to 1500 (2B) and order themselves the way the stabilisation literature says they should. Which path a category rides decides how much that constant controls: `raw` categories are governed by it entirely, the expected-stat blend only through its 40% actual side, and K/BB only when the talent gate fails. That is why the per-knob fit saw the talent layer over-spread worst exactly where the raw path dominates. They all sat at a hand-chosen 100 until 2026-09 — see [history.md](./history.md#2026-09--category-regression-priors-were-4-9x-too-weak-and-were-never-measured).
...
### Layer 3 — Batter score scale

`getBatterRating` composites `100 × Σ (cat.weight × cat.normalized) × opportunity`, where `cat.normalized` maps the L2 expected per-PA rate onto a 0–1 window from `CATEGORY_BASELINE_CONFIG[stat].norm` ([mlb/categoryBaselines.ts](../src/lib/mlb/categoryBaselines.ts)). Tier is derived from score in `batterRating.ts` (great ≥ 70, good ≥ 55, neutral 45–55, poor 30–45, bad below).

**Centering contract (2026-09-08).** Every window is `center ± halfWidth` with `center = leagueMean`, so a league-average rate normalises to exactly 0.5 and a league-average batter in a league-average matchup composites to 50 — the same contract the pitcher side adopted in [2026-07](./history.md#2026-07--pitcher-score-scale-recentered-league-average--50). Before this the batter windows were "floor → elite" guesses (AVG .220–.300 with the league at .239) that put league-average at 0.11–0.39 per category: an average batter-day read 29–38, the tier bands sat ~20 points above the real middle, and when pivotality collapsed the weight vector onto one contested category the day score became `100 × norm(cat) × opportunity` and piled up at 100 for anyone above ~.290 in that category. The `leagueAverage` profile in the batter smoke harness locks the composite to 46–54; move a league mean or a window and the harness says so.

**Half-widths are 2.5 forecast-SDs.** Each `halfWidth` is 2.5 × the standard deviation of the forecast layer's own per-PA output for that category across the 37k-row retro cohort, measured after the knob reliabilities were applied. That makes one SD of forecast movement worth the same score points in every category — the per-cat exchange rate is set by the model's actual spread rather than by hand — and clips roughly 1% of batter-days at either end. Windows bracket what the forecast *produces*, not raw MLB stat ranges: if a window looks wrong, check what `buildBatterForecast` emits for the cohort before touching it (`scripts`-side one-liner in [forecast-verification.md](./forecast-verification.md#per-knob-calibration-fit-2026-09-full-season-retro-cohort)). SB is right-skewed and its symmetric window's floor sits below zero; that is deliberate — a zero-SB forecast normalises to ~0.26, not 0.
...
| `reliableKnob` / `applyKnobs` | [mlb/knobReliability.ts](../src/lib/mlb/knobReliability.ts) | How much of a modifier's raw swing survives to the matchup it is applied to. Every L2 batter knob takes an effect size measured on a different population (league totals, full seasons, three-year park windows) and used on one batter in tonight's posted lineup, so each needs shrinking toward 1.0 the way a thin-sample rate is shrunk toward the league mean — the engine regressed player talent rigorously and its own modifiers not at all, and over 37,167 graded rows 25 of 28 knob coefficients came back below 1.00. Applied as `m ** r` to match the fit's `log(knob)` regressor, so setting a reliability to the fitted coefficient makes the fit read 1.00 on the next pass. `applyKnobs` is the ONLY way `applyMatchupModifier` builds an adjusted rate, which is what keeps the ledger's recorded `context.knobs` and the number the app used from drifting apart. Everything sits at 1.0 today: the mechanism landed behaviour-neutral, and each fitted value is its own reviewable change. Platoon calibrates one level down instead, in `platoon.ts` — its multiplier blends two sources of different reliability, so a knob-level factor would wrongly rescue the player's own observed split along with the population table. |
| `platoonFactor` | [mlb/platoon.ts](../src/lib/mlb/platoon.ts) | Per-category platoon multiplier. Bayesian-regresses the batter's OWN observed vs-hand ratio (`ratiosVsL/ratiosVsR`, plumbed from `getBatterSplits`) toward a population target keyed on (bats, facingHand), weighted by his PA on that side. Per-cat `PRIOR` reflects stabilization (K/BB/AVG/H/R/RBI 1000 PA, TB/HR 1300–1500). The population target is the sourced `PLATOON_COMPONENT` row re-centred on the league PA mix (so it is a pure tilt with no level bias) and then scaled by `PLATOON_TILT_SCALE` — the share of that tilt the retro cohort delivers (2026-09; K 0.60, BB 1.25, rest 0.80, see [forecast-verification.md](./forecast-verification.md#the-platoon-knob)). Applied per-cat inside `buildBatterForecast`, NOT as a composite multiplier. Switch hitters: population target ~1.0, so their observed side-gap regresses against neutral (no special case). Large on K, small on AVG/H, damped on HR; SB has none. Replaced the OPS-based composite `getPlatoonAdjustedTalent` (2026-05). See file header + history.md for sourcing. |
...
| `CATEGORY_BASELINE_CONFIG[*].norm` | [mlb/categoryBaselines.ts](../src/lib/mlb/categoryBaselines.ts) | Batter per-cat windows, `center ± halfWidth`; center = league mean, half-width = 2.5 forecast-SDs over the retro cohort (see [§Layer 3 — Batter score scale](#layer-3--batter-score-scale)). Locked by the batter harness `leagueAverage` profile (46–54) |
